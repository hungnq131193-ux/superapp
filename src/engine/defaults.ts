import type {AreaConfig,DesignInput,Plot,Requirements} from '../types';

export interface Preset {
  name: string;
  patch: {plot?: Partial<Plot>; requirements?: Partial<Requirements>};
}

/** Áp preset lên input mặc định — dùng chung cho UI và test. */
export function applyPreset(preset: Preset): DesignInput {
  const next = structuredClone(defaultInput);
  if (preset.patch.plot) Object.assign(next.plot, preset.patch.plot);
  if (preset.patch.requirements) Object.assign(next.requirements, preset.patch.requirements);
  return next;
}
